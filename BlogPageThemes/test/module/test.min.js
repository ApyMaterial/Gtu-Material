...
async function testGetPaperSolutionList() {
  ...
  async function testGetSubjectsData(subject, book, paper) {
    ...
    for (k of subject) {
      ...
      if (response.status == "200") {
        ...
        <div class="container mx-auto pb-2">
          ...
          testgetSubjectPapers(SubjectData["old_paper"], k);
        } else if ((book = true)) {
          if (response.status == "200") {
            pagediv.innerHTML +=
              `
            ...
            testgetSubjectBooks(SubjectData["Books"], k);
          } else {
            continue;
          }
        }
  ...
  function testgetSubjectBooks(all_books, subject_code) {
      ...
      if(i["buy_link"]) {
        BookSection.innerHTML +=
          ...
          <div class=" justify-center item">
                ...
                i["alt_data"] +
                `" />
                <a target="_blank" href="#" class="btn top-0 right-0 absolute btn-info btn-sm rounded-circle">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="28" viewBox="0 0 48 48"><g fill="none" stroke="currentColor" stroke-width="4"><path stroke-linejoin="round" d="M6 15h36l-2 27H8L6 15Z" clip-rule="evenodd"/><path stroke-linecap="round" stroke-linejoin="round" d="M16 19V6h16v13"/><path stroke-linecap="round" d="M16 34h16"/></g></svg>
              </a> 
                    </div>
                    <div class=" pt-2 m-2">
...
`;
  ...
  function testgetSubjectPapers(all_papers, subject_code) {
  ...
  async function testGetSingleSubjectsData(subjectCode) {
    ...
    if (response.status == "200") {
      ...
      if (SubjectData.syllabus != '') { testSyllabus(SubjectData); }
      if (SubjectData.old_paper.length > 0) { testOldPapers(SubjectData); }
      if (SubjectData.Books.length > 0) { testBooks(SubjectData); }
      if (SubjectData.imp_material != '') { testImpMaterial(SubjectData); }
      if (SubjectData.assignments != '') { testPrecticalAssignment(SubjectData); }
      if (SubjectData.paper_solution != '') { testSubjectPaperSolution(SubjectData); }
  ...
  function testSyllabus(SubjectData) {
  ...
  function testOldPapers(SubjectData) {
    pagediv.innerHTML +=
      `
      <div class="border-2 border-gray-700 rounded-lg shadow-inner-lg mt-2 mb-2">
  <div class="subject_name p-2 align-center  text-white text-center font-revert font-semibold text-lg bg-gray-800">
  ...
  <div class="container mx-auto pb-2">
      <div class="flex flex-wrap -mx-1 overflow-hidden sm:-mx-1 md:-mx-1 lg:-mx-1 xl:-mx-1	" id="OldPapers"> </div>
            ...
            testgetSubjectPapers(SubjectData["old_paper"], 'OldPapers');
  }
  function testBooks(SubjectData) {
  ...
  <div class="container mx-auto pb-2">
      <div class="flex flex-wrap -mx-1 overflow-hidden sm:-mx-1 md:-mx-1 lg:-mx-1 xl:-mx-1	" id="Books"> </div>
            ...
            testgetSubjectBooks(SubjectData["Books"], 'Books');
  }
  function testImpMaterial(SubjectData) {
  ...
  function testSubjectPaperSolution(SubjectData) {
  
    pagediv.innerHTML += `<div class="post-header"><div class="post-head"><h1 class="post-title entry-title" itemprop="name">Paper Solutions </h1></div></div><div class="container"><div class="row"><div class=""><div class="card card-margin"><div class="card-body border-2 "><div class="widget-49"><div class="widget-49-title-wrapper"><div class="widget-49-date-primary"><span class="widget-49-date-day">` + SubjectData.known_as + `</span></div><div class="widget-49-meeting-info"><span class="widget-49-pro-title">` + SubjectData.subject_name + `</span><span class="widget-49-meeting-time">` + SubjectData.subjecet_code + `</span></div></div><a href="` + SubjectData.paper_solution + `?subject_code=`+ SubjectData.subjecet_code + `"
  class="btn btn-sm btn-flash-border-primary widget-49-meeting-action">View</a></div></div></div></div></div></div> `;
  
  }
  function testPrecticalAssignment(SubjectData) {
